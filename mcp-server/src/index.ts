...
server.tool(
  "create_task",
  "Create a new task in Hansei. Use for actionable items the user wants to track.",
  {
  ...
  async ({ user_id, title, priority, due_today }) => {
    ...
    return { content: [{ type: "text" as const, text: `✅ Task created: "${data.title}" (${data.priority}) [#${data.id?.slice(0, 8)}]` }] };
  }
);

server.tool(
  "list_tasks",
  "List the user's tasks. Returns incomplete tasks by default, sorted by most recent.",
  {
...
server.tool(
  "log_idea",
  "Log a new idea in Hansei. Ideas are separate from tasks — they're for brainstorming, not execution.",
  {
    user_id: z.string().uuid(),
  ...
  async ({ user_id, title, body }) => {
    ...
    if (error) return { content: [{ type: "text" as const, text: `Error: ${error.message}` }] };
    return { content: [{ type: "text" as const, text: `💡 Idea logged: "${data.title}" [#${data.id?.slice(0, 8)}]` }] };
  }
);

server.tool(
  "list_ideas",
  "List the user's ideas, sorted by most recent.",
  {
    user_id: z.string().uuid(),
  ...
  async ({ user_id, limit }) => {
    ...
    if (error) return { content: [{ type: "text" as const, text: `Error: ${error.message}` }] };
    if (!data?.length) return { content: [{ type: "text" as const, text: "No ideas found." }] };

    const lines = data.map((i: any) => `💡 ${i.title} [#${i.id.slice(0, 8)}]`);
    return { content: [{ type: "text" as const, text: lines.join("\n") }] };
  }
);
...
server.tool(
  "log_dream",
  "Log a dream entry in Hansei for dream journaling.",
  {
    user_id: z.string().uuid(),
  ...
  async ({ user_id, title, body, emotion }) => {
    ...
    if (error) return { content: [{ type: "text" as const, text: `Error: ${error.message}` }] };
    return { content: [{ type: "text" as const, text: `🌙 Dream logged: "${data.title}" [#${data.id?.slice(0, 8)}]` }] };
  }
);

server.tool(
  "search_items",
  "Search across all item types (tasks, ideas, dreams) by keyword.",
  {
    user_id: z.string().uuid(),
  ...
  async ({ user_id, query, type, limit }) => {
    ...
    if (error) return { content: [{ type: "text" as const, text: `Error: ${error.message}` }] };
    if (!data?.length) return { content: [{ type: "text" as const, text: `No results for "${query}".` }] };

    const icons: Record<string, string> = { task: "⬜", idea: "💡", dream: "🌙" };
...
server.resource(
  ...
  async (uri) => {
    ...
    const text = data?.length
      ? data.map((t: any) => `- [${t.priority}] ${t.title}`).join("\n")
      : "No open tasks.";
