...
const categories = ['work', 'personal', 'health', 'finance', 'home', 'social', 'learning', 'errands', 'fitness', 'medical', 'shopping', 'travel', 'creative', 'email', 'calls', 'meetings', 'coding', 'writing', 'family', 'gifts', 'bills', 'maintenance'];
...
// Category icons (Heroicons style - stroke only)
const CategoryIcon = ({ category, className = "w-4 h-4" }: { category: string; className?: string }) => {
  const icons: Record<string, React.ReactNode> = {
    ...
    fitness: (
      <svg className={className} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M15.362 5.214A8.252 8.252 0 0112 21 8.25 8.25 0 016.038 7.048 8.287 8.287 0 009 9.6a8.983 8.983 0 013-5.386 8.25 8.25 0 013.362 1z" />
...
export default function AppDashboard() {
  ...
  const toggleThemeWithAnimation = () => {
    ...
    // Change theme at midpoint of animation (400ms total)
    setTimeout(() => {
          ...
          <div className="mb-6 p-5 rounded-2xl bg-white dark:bg-[#2c2c2e] border border-gray-200 dark:border-[#38383a] animate-fade-in relative z-40">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 rounded-xl font-semibold bg-[#c8d9cb] hover:bg-[#b8c9bb] active:bg-[#a8b9ab] transition-all text-black"
              >
                Guardar Todo
              </button>
              <button
                    ...
                    <div key={idea.id} className="relative mb-2 overflow-hidden rounded-2xl">
                      ...
                      {!isIdeaSelected && (
                        <div className="absolute inset-0 flex">
                          <div className="flex-1" />
                          <div className={`flex-1 bg-red-800 flex items-center justify-end pr-6 transition-opacity rounded-r-2xl ${showIdeaDelete ? 'opacity-100' : 'opacity-0'}`}>
                        ...
                        {isIdeaSelected ? (
                          <div 
                            ...
                            className={`w-12 h-12 rounded-full flex-shrink-0 flex items-center justify-center cursor-pointer transition-all ${
                              recording 
                                ? 'bg-black dark:bg-white scale-110 ring-2 ring-black/30 dark:ring-white/30 animate-pulse' 
                              ...
                              return (
                                <div key={task.id} className="relative flex items-center">
                                  ...
                                  <div 
                                    ...
                                    isStepSelected
                                      ? recording
                                        ? 'bg-[#c8d9cb] dark:bg-white text-black dark:text-black scale-110 ring-2 ring-[#c8d9cb]/50 dark:ring-white/30 animate-pulse cursor-pointer'
                                  ...
                                  <div 
                                    ...
                                    className={`flex-1 ml-3 p-3 rounded-xl transition-all hover:shadow-md active:scale-[0.98] border-2 ${
                                      isStepSelected
                                        ...
                                        : task.completed 
                                          ? 'bg-white/50 dark:bg-[#2c2c2e]/50 border-transparent active:bg-gray-100 dark:active:bg-[#48484a] active:border-gray-300 dark:active:border-[#68686a]' 
                                          : `${colors.cardBg} ${colors.cardBgDark} border-white dark:border-[#38383a]/50 active:bg-gray-100 dark:active:bg-[#48484a] active:border-gray-300 dark:active:border-[#68686a]`
                                    }`}
                                  >
                    ...
                    <div key={task.id} className="relative overflow-hidden rounded-2xl">
                      ...
                      {!isTaskSelected && (
                        <div className="absolute inset-0 flex">
                          ...
                          <div className={`flex-1 bg-red-800 flex items-center justify-end pr-6 transition-opacity ${showDelete ? 'opacity-100' : 'opacity-0'}`}>
                            <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                            </svg>
                          </div>
                      ...
                      <div
                        ...
                        className={`relative rounded-2xl p-4 border border-gray-200 dark:border-[#38383a] border-2 flex items-center gap-4 group transition-all overflow-hidden active:scale-[0.98] ${
                          isTaskSelected
                            ? 'border-black dark:border-white ring-2 ring-black dark:ring-white/30 scale-[1.02]'
                            : `${colors.cardBg} ${colors.cardBgDark} border-gray-200 dark:border-[#38383a]/50 active:bg-gray-100 dark:active:bg-[#48484a] active:border-gray-300 dark:active:border-[#68686a]`
                        }`}
                      >
                          ...
                          >
                            {isTaskSelected ? (
                              ...
                              <span className="text-gray-500 dark:text-gray-400">
                                <CategoryIcon category={task.category || 'personal'} />
                              </span>
                            )}
                          </div>
            ...
            <div className="mt-8">
              ...
              {completedTasks.map((task) => (
                ...
                >
                  ...
                  <button
                    onClick={() => toggleTask(task.id, task.completed)}
                    className="w-8 h-8 rounded-full bg-[#c8d9cb] flex items-center justify-center"
                  >
